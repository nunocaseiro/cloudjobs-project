const BACKEND_URL = 'http://aee9a617de19844698a0e80e91601c37-383708555.eu-west-2.elb.amazonaws.com'
const ITJOBS_URL = 'http://aee9a617de19844698a0e80e91601c37-383708555.eu-west-2.elb.amazonaws.com'
const STATISTICS_URL = 'http://aee9a617de19844698a0e80e91601c37-383708555.eu-west-2.elb.amazonaws.com'
const GATHERKEYWORDS = 'http://aee9a617de19844698a0e80e91601c37-383708555.eu-west-2.elb.amazonaws.com'
const SEARCH_URL = 'http://aee9a617de19844698a0e80e91601c37-383708555.eu-west-2.elb.amazonaws.com'
...
const createPagination = (meta) => {
  ...
  a.setAttribute('data-page', (meta.page - 1 > 0) ? meta.page - 1 : pages)
  ...
  a.innerHTML = '>';
  a.classList.add('page-link');
  a.setAttribute('data-page', (meta.page + 1 < pages) ? meta.page + 1 : 1)
  a.addEventListener('click', (event) => {
    event.preventDefault();
    const page = event.target.getAttribute('data-page');
    let searchTerms = document.querySelector("#search").value;
...
const queryAPI = (search, local = false, page = 1, limit = 10) => {
  return new Promise((resolve, reject) => {
    url = `${ITJOBS_URL}/api/itjobs/search`;
    if (local) url = `${SEARCH_URL}/api/search/local`;
    const parameters = {
      search: search,
      page: page,
      limit: limit
...
const newKeywordListener = (e) => {
  ...
  axios.post(`${GATHERKEYWORDS}/api/keywords/insert`, { data: keyword })
    .then(response => {
      getExistingKeywords();
...
const getExistingKeywords = () => {
  ...
  axios.get(`${GATHERKEYWORDS}/api/keywords/`).
    then(response => {
      ...
      for (let keyword of response.data) {
        ...
        button.addEventListener('click', (e) => {
          ...
          axios.delete(GATHERKEYWORDS+'/api/keywords/' + keyword)
            .then(response => {
